...
이 문서는 `pytorch-ir`로 PyTorch 모델에서 IR를 추출할 때 필요한 핵심 개념을 설명합니다.

## 1. 개요

### 1.1 목적

`pytorch-ir`는 PyTorch 모델에서 중간 표현(IR)을 추출합니다. 핵심 목표는 다음과 같습니다:
...
| Executor | `executor.py` | 검증과 확인을 위한 IR 그래프 실행 |
...
- **완전성**: 모든 연산이 명시적으로 표현
- **저수준 일관성**: 모델 계산을 명시적으로 표현하는 안정적인 표현
